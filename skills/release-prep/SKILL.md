...
**15. Commit, push, open PR.** PR body must open with an AI-generated-content marker, then the same drafted-changelog/flagged-items content as the final summary (minus CI status, which isn't known yet), and close with a one-line skill attribution/link for traceability:
```
...
---
:robot: Generated by the `release-prep` skill (v<metadata.version from frontmatter, e.g. 1.0.0>) — <link to the skill's source, e.g. a marketplace repo or `.claude/skills/release-prep/SKILL.md` in this repo>. Something look wrong? Flag it there.
```
```bash
...
- When editing this skill's own workflow, bump `metadata.version` in the frontmatter above (semver) — the PR footer reports it for traceability.
